...
**EH-1~EH-27 + EH-29~EH-39 중 15건 완료 · 1건 일부 완료 · 22건 보류.** (EH-28은 아래 단서 참조)
...
| 🕗 보류 | EH-4 · EH-8 · EH-10 · EH-17 · EH-20 · EH-21 · EH-22 · EH-23 · EH-24 · EH-25 · EH-26 · EH-27 · EH-29 · EH-31 · EH-32 · EH-33 · EH-34 · EH-35 · EH-36 · EH-37 · EH-38 · EH-39 |
...
- **트리거 대기**: EH-17(오버레이 누적 성장 실측) · EH-20 · EH-23 · EH-25(1.0 이전 runtime 관찰) ·
  ...
  EH-38(peer 실측 기반 예산 기본값 재보정) · EH-39(리뷰어 CLI 동작 중 미확인 항목).
- **수용된 잔여 위험**: EH-21 · EH-22 · EH-24 · EH-26 · EH-27 · EH-29 · EH-33 · EH-34 · EH-36 · EH-37. 사이클이 범위를 그으며 명시
...
`SAGE - 감사 조회 필터 정화 충돌 잔여 위험 (EH-28, 26.08.27)`이 소유한다. 번호만 보고 EH-27까지가
전부라고 읽지 않도록 이 줄을 남긴다.
...
- **상태**: 🕗 **보류(2026-07-30, §10-g에서 분리)**. §10-g 완료 후 독립 L3 설계·검토 대상으로 유지한다.
  `sage-audit-visibility-no-vault`(2026-08-27)는 이 한계를 **해소하지 않고 표면화**했다. `sage audit
  show`가 보증을 `method`×`status` 두 축으로 나눠, 위변조 내성이 없는 세 로그가 어떤 경로로도
  `valid`로 표시되지 않는다. 조회가 정직해진 것이지 로그가 강해진 것이 아니다.
...
## EH-29 — 드리프트한 전역 SAGE skill 사본을 지울 수단이 없다

- **배경**: `sage uninstall --global`·`--all`은 전역 배포본이 프로젝트 렌더와 hash·bytes로 일치할 때만 지운다. 사용자가 손댄 사본은 소유권을 증명할 수 없으므로 `PRESERVE`로 남긴다. 이것은 이 명령의 설계 전제("소유권을 추측하지 않는다")가 의도한 결과다.
- **문제**: 그래서 한 번 드리프트한 사본은 SAGE가 제공하는 어떤 경로로도 지워지지 않는다. 경로와 사유는 보고되므로 사용자가 직접 지울 수 있지만, 도구로는 영구히 남는다.
- **접근**: `--force`나 소유권 우회 옵션을 더하는 것은 기각했다 — 그 옵션 하나가 "추측해서 지우지 않는다"는 전제를 통째로 뒤집고, 잘못 지우는 방향의 사고는 되돌릴 수 없다. 대신 (1) manifest가 설치 시점 소유권을 증명하게 만들어 드리프트 여부를 사후가 아니라 사전에 판정하거나, (2) 전역 배포본에 프로젝트 결속 receipt를 남겨 "이 프로젝트가 배포한 것"을 hash 일치와 독립적으로 증명하는 두 방향을 검토한다.
- **규모/위험**: 중간. manifest schema나 전역 배포 경로의 계약을 바꾸므로 install·generate·uninstall 셋을 함께 봐야 한다.
- **트리거**: `--force` 요구가 실제로 들어오거나, manifest가 설치 전 소유권을 증명하게 되거나, 드리프트 사본이 다른 프로젝트를 실제로 깨뜨린 사례가 관측될 때.
- **상태**: 🕗 **보류(2026-08-27, `sage-uninstall` Phase 02 RU-2로 등재)**. severity P2 — 잘못 지우는 방향의 위험은 0이고 못 지우는 방향으로만 남는다. 사용자 승인 수용이며 **해소가 아니다.** owner: uninstall maintainer.

---

## EH-30 — Windows에서 `sage uninstall`이 상위 디렉터리 교체 경쟁을 막지 못한다

- **배경**: `sage uninstall`의 모든 변경은 첫 mutation 전에 연 **부모 디렉터리 fd**에 결속된다(`openat`·`renameat`·`unlinkat` 계열). 그래야 상위 이름이 실행 도중 symlink로 바뀌어도 작업이 원래 디렉터리로 간다. `O_NOFOLLOW`는 마지막 성분만 보므로 이것 없이는 상위 교체를 막지 못하고, 실제로 프로젝트 밖에 파일이 만들어지는 것을 재현했다.
- **문제**: Windows는 `dir_fd`를 제공하지 않는다. 그래서 현재 구현은 그 플랫폼에서 **mutation 전에 `uninstall.unsafe_platform`으로 거부**한다. 계획(`--check`)은 읽기라 그대로 동작하지만, Windows 사용자는 `sage uninstall`로 자산을 제거할 수 없다.
- **접근**: `CreateFileW`(`FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT`)로 디렉터리 handle을 열고 `SetFileInformationByHandle`(`FileRenameInfoEx`의 root directory handle)로 handle 기준 rename을 수행하는 방향. 경로 재검사로 대체하지 않는다 — 검사와 작업 사이 창이 그대로 남고, 그 창이 정확히 공격 지점이다.
- **규모/위험**: 큼. ctypes 기반 플랫폼 전용 코드이고 Windows 러너에서만 검증된다. 이 사이클에서 넣지 않은 이유가 그것이다 — **검증할 수 없는 플랫폼 전용 코드는 막으려는 위험보다 나쁘다.**
...
## EH-36 — codex 리뷰어의 하위 에이전트 생성을 설정으로 막을 수 없다

- **배경**: 1.2.0 은 Phase 05 리뷰어를 통제된 프로세스로 띄운다. claude 는 `--disallowedTools Agent Task` 로 하위 에이전트 도구를 없앤다.
- **문제**: codex 0.155.1 은 `--disable multi_agent`·`multi_agent_v2`, `features.*`, `collaboration_modes`, `code_mode_host`, `agents.max_depth=0`, `agents.max_threads=1` 어느 것으로도 `collaboration.spawn_agent` 가 사라지거나 생성이 거부되지 않았다(실측, codex 확인 — `max_depth` 는 V1 전용이라 V2 자식에 무시). 하위 에이전트 생성이 `--json` stdout 에 안 남는 실행도 있었다.
- **수용 근거**: 세션 기록의 `parent_thread_id` 를 재귀로 따라 자손을 찾고, 사용량을 합산하고, `REVIEWER_AUDIT: warn subagent_spawned:N` 으로 드러낸다. 막을 수단이 없는 행동이라 강등하지 않는다(강등하면 재실행만 반복). 리뷰어 계약이 생성을 금지하고, 1.2.0 개발 중 codex 리뷰 4회에서는 생성 0회였다.
- **접근(재검토 시)**: codex 가 하위 에이전트를 끄는 설정을 제공하면 통제 argv 에 추가하고, 관측 시 `violation`(강등)으로 올린다.
- **트리거**: codex 릴리스에 해당 설정이 생길 때, 또는 실사용에서 생성이 반복 관측될 때.
- **상태**: 🕗 보류 — 수용된 잔여 위험(2026-09-19).

## EH-37 — 리뷰어 하위 프로세스 정리의 잔여 경쟁

- **배경**: 리뷰어가 띄운 셸 명령은 codex 에서 명령마다 새 세션으로 뜬다(실측). 1.2.0 은 실행 중 최대 2초 간격으로 자손을 표본 수집해 두고, 어느 종료 경로에서든 그 목록과 현재 자손을 함께 종료한다.
- **문제**: (a) 리뷰어가 새 세션 자식을 띄우고 표본 간격 안에 종료하면, 그 자식은 수집 전에 부모가 바뀌어 남는다. macOS 에는 고아를 거두는 subreaper 가 없어 원리적으로 없앨 수 없다. (b) Windows 는 실행 중 자손 수집이 없어, 리뷰어가 정상 종료하면 백그라운드 자식이 **항상** 남고 그 자식이 출력 파이프를 쥐고 있으면 종료가 약 20초 늦어진다(2026-09-19 Windows 11 SSH 실측). timeout 경로는 `taskkill /T` 로 자식까지 종료된다(실측).
- **수용 근거**: 리뷰어가 백그라운드 명령을 남기고 끝나야 생기는 좁은 조건 · 게이트 우회나 결과 손실이 아닌 정리 누락 · 리뷰어 계약이 테스트·빌드 실행을 금지(codex 는 사후 경고) · 1.1.1 은 timeout 경로에서도 자식을 남기고 파이프 EOF 까지 대기했으므로 퇴행이 아니다.
- **접근(재검토 시)**: Linux 는 `PR_SET_CHILD_SUBREAPER` · Windows 는 Job Object 로 트리 전체를 묶어 종료하거나, 최소한 `CreateToolhelp32Snapshot` 으로 실행 중 자손 수집을 더한다(약 1시간).
- **트리거**: 리뷰 뒤 남은 프로세스가 실사용에서 관찰될 때, 또는 EH-31 에서 Windows 자동 검사를 둘 때.
- **상태**: 🕗 보류 — 수용된 잔여 위험(2026-09-19, codex 확인 리뷰에서 지적).

## EH-38 — `budget_tokens` 기본값이 리뷰어 실측을 반영하지 않는다

- **배경**: 1.2.0 부터 예산 판정은 호스트 누적 + 리뷰어의 캐시 밖 입력·출력 합이다. 템플릿 기본값(L2 15만 · L3 60만)은 호스트 추정치만 있던 때 정한 값이다.
- **문제**: 리뷰어 실측이 더해지면서 같은 기본값이 이전보다 일찍 예산 초과로 멈출 수 있다. 1.2.0 개발 중 codex 리뷰 한 번의 캐시 밖 입력은 약 8만이었다.
- **접근**: ChatForYou 수용 기준 측정과 실사용 `peer_usage` 기록이 쌓인 뒤 기본값과 `sage-init` 인터뷰 안내를 다시 정한다. 캐시 읽기를 합산에 넣을지도 그때 판단한다(현재는 기록만).
- **트리거**: 수용 기준 측정 완료, 또는 예산 초과 종료가 실사용에서 반복될 때.
- **상태**: 🕗 보류(2026-09-19 등재).

## EH-39 — 리뷰어 CLI 동작 중 미확인으로 남은 것

- **문제**: (a) claude 사용 한도 소진의 실제 출력 문구를 확인하지 못해 `rate_limit_event` 와 결과 문구로만 판별한다 — 못 잡으면 `exit_nonzero`(폴백 대상은 같음). (b) `~/.codex/AGENTS.md`(전역) 주입이 `project_doc_max_bytes=0` 으로 억제되는지 확인하지 못했다(실측 환경의 파일이 비어 있음). (c) Windows 에서 `--setting-sources=` 한 토큰 전달과 프로세스 트리 종료가 실행 검증되지 않았다.
- **접근**: 해당 상황이 생길 때 출력 원문을 채집해 판별 규칙과 문서를 맞춘다. (c) 는 릴리스 전 SSH 실기 실행 항목으로 다룬다.
- **트리거**: 한도 소진 실사례 · 전역 AGENTS.md 를 쓰는 사용자 보고 · Windows 실기 실행.
- **상태**: 🕗 보류(2026-09-19 등재).
