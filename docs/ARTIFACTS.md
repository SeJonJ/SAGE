...
| 위치 | 성격 | 대표 산출물 | 생성 주체 |
|---|---|---|---|
| `<root>/.sage/` | PDCA 실행 정본 (커밋 대상) | `plan_interview.md` · `knowledge_scan.md` · `loop_audit.jsonl` · `override.jsonl` | CLI · 스킬 |
...
CORE 부트스트랩 자산(6 에이전트·7 스킬)은 `sage install`이 손으로 배포하고 `--force`가 덮어씁니다.
