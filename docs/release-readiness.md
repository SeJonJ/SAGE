...
> **현재 상태 (1.2.0 기준).** 이 job 은 저장소에 등록된 러너가 없어 **실행된 적이 없다** — PR 마다
...
> 같은 스크립트를 직접 실행한 기록이다. 1.2.0 은 Phase 05 리뷰어 프로세스 경로(timeout 시 자식 종료 ·
> 입력을 읽지 않는 리뷰어의 timeout · UTF-8 입출력 · 실제 claude/codex 통제 인자)를 같은 방식으로
> 실행했다. 릴리스 전 이 실행을 사람이 챙겨야 하며, job 을 없앨지·가벼운 hosted 검사로 바꿀지·SSH
> 증거를 릴리스 게이트로 만들지는 후속 과제로 남아 있다.
