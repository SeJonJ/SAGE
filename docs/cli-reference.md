...
**실제 제거의 지원 범위는 POSIX 와 Windows 10/11 로컬 NTFS 입니다.** 이 명령의 안전성은 첫 변경
전에 대상의 부모 디렉터리를 열어 붙드는 데서 나옵니다 — 그 뒤에는 상위 경로 이름이 어떻게
바뀌어도 작업이 원래 디렉터리로 갑니다. 그 결속을 만들 수 없는 환경(네트워크·UNC 경로, NTFS 가
아닌 볼륨, 필요한 native 기능 부재)에서는 **첫 변경 전에** `uninstall.unsafe_platform` 으로
거부합니다. 계획(`--check`)은 읽기라 어디서나 그대로 동작합니다.
