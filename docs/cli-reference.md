...
상위 디렉터리가 실행 도중 바뀌는 경쟁을 막을 수단이 없는 플랫폼(현재 Windows)에서는 **실행을
거부**합니다 — 계획은 `--check` 로 그대로 볼 수 있습니다. 그런 곳에서 그냥 실행하면 프로젝트
밖 파일을 만들 수 있고, 되돌릴 수 없는 명령에서 그 위험은 안내문으로 대신할 수 없습니다.
