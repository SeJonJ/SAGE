...
host 택1이라 호스트별 위치가 다름: claude=repo `.claude/skills/{sage-init,sage-init-local,sage-cycle,sage-plan,sage-team,sage-cycle-fast,sage-plan-fast,sage-team-fast,sage-review,sage-asset,sage-asset-override,sage-profile-modify,sage-feedback}` +
...
- hook 단일소스 = spec md(정책/등록/테스트) + 정본 알고리즘. form=`core_adapter`(대부분: `{id}_core.py` pure core
  + `adapters/{claude,codex}/{id}.sh`). `native`는 legacy/custom POSIX hook 호환 형식이다.
