...
리포트 생성은 항상 하되, retro_gate(9-C) enforce 가 미완료 사이클을 잡으면 **claude host 는 exit 2 로 종료를
1회 막는다**(stop_hook_active 재시도에선 통과 — 플랫폼 제약). codex host 는 리포트에 BLOCK 을 남기되 차단은
안 한다(codex Stop lifecycle 미검증). enforce 미설정(기본 off)이면 종전대로 항상 exit 0.
...
- exit_code: 기본 0. retro_gate enforce + 미완료 + claude host + 첫 Stop → 2(종료 1회 차단, 기존 PreToolUse exit-2 관례 재사용)
...
- policies/retro_gate.py: **공유(양 host)**, 9-C Loop C 게이트. `pdca.retro.report_gate_enforce`(off|advisory|enforce)로
  `sage retro --check` 실행 여부를 세션 종료 시 사후 확인. enforce 만 exit 2(claude). retro_audit.jsonl(감사 트레일)에
  성공(retro_check_ok)·미완료(retro_check_missing)를 append. **enforcement 라 hook_runtime_hash 로 추적**(advisory 인
  위 둘과 달리 부재 시 게이트가 조용히 무동작하므로).
...
- retro_gate(9-C): enforce 첫 Stop exit 2 / stop_hook_active 재시도 통과 / advisory·off 미차단 / 세션스코프·표준 ** glob·
  Loop-Run 결속·멀티마커 skip / codex 미차단 / retro_audit ok·missing 이벤트 + 상태변화 dedup
