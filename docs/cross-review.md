...
확인되면 통제 없이 실행하고 `REVIEWER_CONTROLS: none`으로 알립니다. 제한 시간·실패·중단으로 끝나면 리뷰어가
띄운 하위 프로세스까지 함께 종료합니다. 리뷰어가 백그라운드로 띄운 명령을 남긴 채 정상 종료한 경우에는,
macOS·Linux는 실행 중에 모아 둔 자손 목록으로 정리하고 Windows는 그 목록이 없어 남을 수 있습니다.
