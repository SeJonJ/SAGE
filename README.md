...
- **`sage-review` 스킬**이 루프 본문(찾기/반박/수정)과 **종료 판단**을 host에서 문서화된 규칙대로 수행하고(advisory-first), **`sage review-loop`** CLI가 라운드별 감사를 `.sage/loop_audit.jsonl`에 기록하며 결과/사유 어휘·run_id·**시퀀스 연속성**을 검증합니다. 의도한 리뷰어(`reviewer_requested`)와 실제(`reviewer_actual`)가 어긋나면 `degraded`로 표면화해 — cross-model 요청이 조용히 same-runtime으로 폴백하는 것을 감지 — `report_gate_enforce`(기본 advisory)에 따라 WARN/BLOCK합니다. 종료 backstop은 기존 report←approve(06←05 APPROVED) 그대로 — 루프는 절대 이를 우회하지 않습니다.
...
`sage install`이 hand-ship하는 부트스트랩 스킬입니다. PDCA 사이클을 여는 진입점이 **`/sage-cycle` → `/sage-plan` → `/sage-team`** 3분할로 나뉘어, 전체 구동·기획·개발을 각각 또는 우산 하나로 호출할 수 있습니다.

| 스킬 | 역할 |
|---|---|
| `sage-init` | 설치 후 `project-profile.yaml`을 대화로 부트스트랩 |
| `sage-cycle` | PDCA 00–06 전체를 한 진입점에서 구동(우산) — `sage-plan`→`sage-team` 위임, 게이트는 재구현하지 않음 |
| `sage-plan` | 기획 00–02: plan 문서 + 파일 소유권 맵 |
| `sage-team` | 개발 03–06: 구현 → 결정론 검증 → QA → Phase 05 리뷰 → 완료 |
| `sage-review` | Phase 05 리뷰 라우팅(`cross_model` → `sage cross-check`, 아니면 `sage review`) + 적대적 루프 |
| `sage-asset` | hook/agent/skill 자산 추가·수정(대화 → `sage generate`) |
| `sage-profile-modify` | profile 값 대화형 수정(review loop·게이트 강도 등) |

> claude host는 repo `.claude/skills/`, codex host는 전역 `$CODEX_HOME/skills/`에 설치됩니다(hand-shipped CORE 자산 — write-guard 면제, manifest 비추적). 스킬 본문은 `docs/sage_harness/skills/{id}.md` reference spec을 참조합니다.
...
```bash
git tag v0.9.3          # 패치 단위(0.0.1)로 증가
