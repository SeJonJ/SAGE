...
| `sage sync-overlays` | CORE 렌더의 overlay 관리 블록과 profile 라우팅 블록만 재수렴 (CORE base는 건드리지 않음) |
...
확인합니다(내용 깊이는 게이트 밖, 누락 시 WARN·비차단). 골격 대상이 아닌 노트는 `--skip-structure-check`로 끕니다.
...
프로젝트는 아키텍처 노트·보안 정책·도메인 프로토콜 같은 자기 소유 문서를 갖지만, 에이전트가 세션 시작에 읽는 `AGENT_GUIDE.md`에서는 그 존재를 알 방법이 없었습니다. `governance_docs`는 이 문서들을 profile에서 선언하면 결정론적 **프로젝트 라우팅 블록**으로 `AGENT_GUIDE.md`에 주입해, 에이전트가 시작 시 어떤 문서를 읽어야 하는지 발견하게 합니다.
...
- **경로 포인터 + 짧은 라벨만** 렌더합니다. 분류 trigger(`path_globs`/`content_keywords`)는 절대 담기지 않습니다 — 그것들은 risk hook이 소유하는 authoritative 데이터입니다.
- `doc`은 프로젝트 상대 경로여야 하며 실재해야 합니다(`.github/` 등 숨김 경로 허용, 경로 탈출·절대경로 차단). `label`은 80자 이하 한 줄.
- 공유 정책이라 local profile에서 덮어쓸 수 없고, 값을 바꾸면 `sage sync-overlays`로 라우팅 블록을 다시 수렴시킵니다.
...
- `sage doctor`로 가용성을 확인하세요. 반대 런타임에 도달 실패 시 Phase 05는 `BLOCKED`로 종료됩니다 —
  same-runtime 실행은 `policy: off` 또는 `recommended`의 명시적 local opt-out 경로에서만 정상 완료로 인정됩니다.
- `cross_model.effort`(peer 어휘: codex=`minimal~xhigh`, claude=`low~max`, 미설정 시 `high`)와
  `cross_model.reviewer`(host는 active host 반대여야 하고 model은 peer CLI에 그대로 전달, 생략 시 peer default)는
  `options.cross_model: true`일 때만 유효합니다.
- `sage models --host codex`는 로컬 cache 기반 `cache-confirmed`, `--host claude`는 안정적인 계정 모델 목록
  명령이 없어 `syntax-only/account-unverified`입니다 — SAGE는 모델 확인을 위해 유료 요청을 자동 실행하지 않습니다.
