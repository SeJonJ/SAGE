...
sage generate   # spec → 등록 산출물(settings.json/hooks.json) + {host}/hooks shim + profile 컴파일 + manifest 스탬프
                ...
                # --kind mcp:    docs/sage_harness/mcps/{id}.md → .mcp.json(claude) + .codex/config.toml managed-block(codex)
...
> form: `core_adapter`(순수 코어 + 런타임 어댑터 — 대부분) / `native`(단일 `.sh` — write-guard).
...
| kind | 생성 성격 | SSOT | 산출물 |
|---|---|---|---|
| `hook` | 결정론(순수 함수) | `docs/sage_harness/hooks/{id}.md` + `{id}_core.py` | settings.json/hooks.json + shim |
| `agent` / `skill` | interpretive(런타임 AI 렌더) | `…/{agents,skills}/{id}.md` + `{id}.claims.yml` | `.claude/.codex` 의 렌더 .md |
| `mcp` | **결정론(선언 데이터 직렬화)** | `…/mcps/{id}.md`(frontmatter payload) | `.mcp.json`(claude) + `.codex/config.toml` managed-block(codex) |

> **mcp**(4번째 kind): MCP 서버 정의를 spec-SSOT 로 거버넌스. payload(transport·command·args·env 변수명·runtime_targets)는 spec md frontmatter 가 SSOT, `profile.mcp.enabled` 는 enable 토글. 시크릿은 env **변수명만**(값 금지) → 생성 시 `${VAR}` placeholder, 리터럴 시크릿은 산출 전 FAIL(fail-closed). `.mcp.json` 은 SAGE 소유(write-guard), `.codex/config.toml` 은 managed-block 만 소유(비-MCP 설정 보존). 정본 `plan_docs/mcp-asset-kind-plan.md`.
...
4. **EH-1/EH-2/wheel 완료** — 동적 roster(`--kind roster` → `implementer-<comp>` spec 결정론 scaffold) + output_contract
   독립화(스택 토큰 0) + 순수 wheel 단독 배포(빌드 시 `_bundle/` 번들, `sage_root` 감지, CI 강제). 각 항목 변이 teeth + 회귀 PASS + CI green.
5. **대화형 부트스트랩 저작 레이어 박제** (커밋 `3795b40`, CI green) — `docs/agent/bootstrap-authoring.md` 신설:
   인터뷰 → profile 값 채움 → `generate`/`validate` 핸드오프 → phase-first 순서를 정형 프로토콜로 기술.
   `AGENT_GUIDE.md` "Project Bootstrap" 섹션 배선. codex 3R cross-model 리뷰 후 사실오류 4건 수정.
   install `_copy_tree(docs/agent)` 자동 배포 + wheel 번들 포함 검증.
6. **MCP 4번째 자산 kind 완료** (2026-06-20) — MCP 서버 정의를 hook·agent·skill 에 이은 결정론 자산으로 편입.
   `sage/mcp_common.py`(파싱·직렬화·시크릿 문법) + `_gen_mcp`/`_validate_mcp` + write-guard `.mcp.json` +
   profile/manifest 스키마(`form: declarative`). ChatForYou 실 codegraph+obsidian 정의를 fixture 로 한
   **shadow 파일럿**(라이브 무변경) e2e. **codex 6R cross-model 리뷰**(P0 3·P1 8·P2 2 전부 해소, 최종 SHIP).
...
1. ✅ **잔여 open 엔진 항목** — EH-1 동적 roster · EH-2 output_contract 독립화 · wheel 순수 단독배포 (CI green)
2. ✅ **weatherapp 2차 = 대화형 부트스트랩 저작 흐름 + Tier 2 재구축** — 유저 대화 → AI가 SAGE 규칙대로
   profile/spec 저작 → `generate`/`validate` 핸드오프. `bootstrap-authoring.md` 박제 + weatherapp e2e 실증
   (backend 14 + frontend 2 tests, 게이트 teeth, codex cross-model APPROVED). 영속 딜리버러블 = 커밋 `3795b40`.
3. ✅ **MCP 개발** — MCP 서버를 4번째 거버넌스 자산 종류로 편입(결정론). Standalone(framework lift) +
   Enhancement(ChatForYou codegraph+obsidian shadow 파일럿, 라이브 무변경). codex 6R 리뷰 SHIP.
   설계 정본: vault `SAGE-MCP 개발` · 구현 정본: `plan_docs/mcp-asset-kind-plan.md`.
4. **(다음) weatherapp 3차** — MCP 자산 클래스 포함 end-to-end 실증 (engine-N + MCP 베이스라인)
