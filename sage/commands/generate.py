...
def _build_registration(root, target, hook_ids, project_hooks=None):
    ...
    _layout = asset_paths.detect_layout(root)
    missing = []
    ...
    for hid in sorted(hook_ids):   # lexicographic 정렬(결정론)
        if not valid_hook_id(hid):
            ...
            continue
        ap = AssetPaths(root, "hook", hid, layout=_layout)
...
def _shim_body(target, hook_id, form, hooks_rel):
    ...
    canon = (f'$ROOT/{hooks_rel}/adapters/{target}/{hook_id}.sh'
             if form != "native" else f'$ROOT/{hooks_rel}/{hook_id}.sh')
    return (
        "#!/usr/bin/env bash\n"
        "# generated by sage generate — do not edit. canonical: " + hooks_rel + " (single source).\n"
        f'ROOT="${{SAGE_PROJECT_ROOT:-${{{root_env}:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}}}}"\n'
        'export SAGE_HOOK_CORE_DIR="$ROOT/' + hooks_rel + '"\n'
...
def _write_hook_shims(args, root, manifest, hook_ids, target):
    ...
    for hid in sorted(hook_ids):
        ...
        canon = (os.path.join(asset_paths.layout_hooks_dir(root, _layout), "adapters", target, f"{hid}.sh")
                 if form != "native" else os.path.join(asset_paths.layout_hooks_dir(root, _layout), f"{hid}.sh"))
        if not os.path.exists(canon):
            continue
        p = os.path.join(hooks_dir, f"{hid}.sh")
        Path(p).write_text(_shim_body(target, hid, form, _hooks_rel), encoding="utf-8")
        os.chmod(p, 0o755)
        written += 1
    print(tr(language_of(args), 'cli.generate.msg01', target=target, written=written, os_path=os.path.relpath(hooks_dir, args.dest)))


def _compile_profile(root, dest, language=None):
    ...
    try:
        ...
        print(tr(language, 'cli.generate.msg02'), file=sys.stderr)
        return "fail", None, None, None
    except Exception as e:
        print(tr(language, 'cli.generate.msg03', arg=type(e).__name__, e=e), file=sys.stderr)
        return "fail", None, None, None
...
def _host_registration_body(target, outp, registration, language=None):
    document = {"hooks": registration}
    if os.path.exists(outp):
        try:
            existing = json.loads(Path(outp).read_text(encoding="utf-8"))
        except Exception as exc:
            raise ValueError(tr(language, "cli.generate.host_registration_corrupt",
                                target=target, exc=exc)) from exc
        if not isinstance(existing, dict):
            raise ValueError(tr(language, "cli.generate.host_registration_root_not_object",
                                target=target))
...
def _stamped_manifest(root, manifest, hook_ids, runtime_hash):
    ...
    for hid in hook_ids:
        ...
        if not entry:
            continue
        paths = AssetPaths(root, "hook", hid, layout=_layout)
...
def _gen_hook_locked(args, root):
    _layout = asset_paths.detect_layout(root)
    _hooks_rel = os.path.relpath(asset_paths.layout_hooks_dir(root, _layout), root)
    ...
    if not isinstance(manifest, dict) or not isinstance(manifest.get("assets"), dict):
        print(tr(language_of(args), 'cli.generate.msg06'), file=sys.stderr)
        return 2
    ...
    for hook_id in all_hook_ids:
        paths = AssetPaths(root, "hook", hook_id, layout=_layout)
        registered_inputs.extend((paths.spec, paths.core, paths.native,
                                  paths.adapter("claude"), paths.adapter("codex")))
    for runtime_paths in hook_runtime_files(root, _layout).values():
    ...
    if args.id and args.id not in all_hook_ids:
        ...
        core_candidate = os.path.join(asset_paths.layout_hooks_dir(root, _layout),
        ...
        if not (os.path.lexists(spec_candidate) or os.path.lexists(core_candidate)):
            print(tr(language_of(args), 'cli.generate.msg07', args_id=args.id), file=sys.stderr)
            return 2
        if args.write and args.target != "both":
            print(tr(language_of(args), 'cli.generate.msg08'),
                  file=sys.stderr)
            return 2
    ...
    for hid in sorted(all_hook_ids):
        ...
        paths_for_id = AssetPaths(root, "hook", hid, layout=_layout)
        if not isinstance(entry, dict):
            print(tr(language_of(args), 'cli.generate.msg09', hid=hid),
                  file=sys.stderr)
            return 2
        has_project_sources = (
        ...
        if has_project_sources and entry.get("form") != "core_adapter":
            print(tr(language_of(args), 'cli.generate.msg10', hid=hid), file=sys.stderr)
            return 2
        ...
        if entry.get("form") != "core_adapter":
            print(tr(language_of(args), 'cli.generate.msg11', hid=hid),
                  file=sys.stderr)
            return 2
        project_paths = AssetPaths(root, "hook", hid, layout=_layout)
        ...
        for runtime in ("claude", "codex"):
            ...
            if os.path.lexists(adapter_path):
                if (not os.path.isfile(adapter_path) or os.path.islink(adapter_path)
                    ...
                    print(tr(language_of(args), 'cli.generate.msg12', adapter_path=adapter_path),
                          file=sys.stderr)
                    return 2
            else:
    ...
    if args.id:
        if args.id not in all_hook_ids:
            print(tr(language_of(args), 'cli.generate.msg13', args_id=args.id), file=sys.stderr); return 2
        stamp_ids = [args.id]
    else:
    ...
    profile_status, profile_data, profile_body, profile_path = _compile_profile(
        root, args.dest, language_of(args))
    runtime_hash = None
    if args.write:
        status = profile_status
        if status == "fail":
            print(tr(language_of(args), 'cli.generate.msg14'),
                  file=sys.stderr)
            return 1
        ...
        runtime_hash, missing_runtime = calculate_hook_runtime_hash(root, _layout)
        if missing_runtime:
            print(tr(language_of(args), 'cli.generate.msg16') +
    ...
    if args.write and engine_tree:
        print(tr(language_of(args), 'cli.generate.msg17'))
    ...
    for tgt in targets:
        ...
        if missing:
            print(tr(language_of(args), 'cli.generate.msg18', tgt=tgt, items=', '.join(missing)), file=sys.stderr)
            return 1
        ...
        try:
            body = _host_registration_body(tgt, outp, reg, language_of(args))
        ...
        for hid in sorted(reg_ids):
            form = manifest["assets"].get(f"hooks/{hid}", {}).get("form", "core_adapter")
            canonical = (os.path.join(asset_paths.layout_hooks_dir(root, _layout), "adapters", tgt, f"{hid}.sh")
                         if form != "native" else os.path.join(asset_paths.layout_hooks_dir(root, _layout), f"{hid}.sh"))
            if hid in project_hooks or os.path.exists(canonical):
                shim_path = os.path.join(args.dest, _RUNTIME_DIR[tgt], "hooks", f"{hid}.sh")
                shims.append((shim_path, _shim_body(tgt, hid, form, _hooks_rel)))
    ...
    try:
        ...
        if cleanup_errors:
            print(tr(language_of(args), 'cli.generate.msg19') + "; ".join(cleanup_errors),
                  file=sys.stderr)
    except (OSError, ValueError, InstallBusyError, InstallDriftError) as exc:
        rollback_errors = transaction.rollback() if transaction is not None else []
        suffix = (tr(language_of(args), "cli.generate.rollback_warning")
                 + "; ".join(rollback_errors)) if rollback_errors else ""
        print(tr(language_of(args), 'cli.generate.msg20', arg=type(exc).__name__, exc=exc, suffix=suffix),
              file=sys.stderr)
        return 1
    if profile_status == "ok":
        print(tr(language_of(args), 'cli.generate.msg21', os_path=os.path.relpath(profile_path, args.dest)))
    if not engine_tree:
        for tgt, reg, outp, _body, shims in planned:
            print(tr(language_of(args), 'cli.generate.msg22', tgt=tgt, os_path=os.path.relpath(outp, args.dest), arg=sum(len(v) for v in reg.values())))
            print(tr(language_of(args), 'cli.generate.msg23', tgt=tgt, count=len(shims), arg=_RUNTIME_DIR[tgt]))
    print(tr(language_of(args), 'cli.generate.msg24'))
    return 0
...
def _seed_from_existing(args, dest, src_id, comp_id, paths):
    ...
    for src, out in zip(src_paths, new_paths):
        ...
        if os.path.exists(out):
            notes.append(tr(language_of(args), "cli.generate.seed_skip_existing",
                            path=os.path.relpath(out, dest)))
            continue
        ...
        for token in residuals:
            notes.append(tr(language_of(args), "cli.generate.seed_residual_identity",
                            path=os.path.relpath(out, dest), token=token))
    if not seeded_any and not notes:
        return [], notes, tr(language_of(args), "cli.generate.seed_no_source_render", src_id=src_id)
...
def _gen_roster(args, root):
    ...
    print(tr(language_of(args), 'cli.generate.msg25'))
    if prof is None:
        print(tr(language_of(args), 'cli.generate.msg26'), file=sys.stderr)
        return 1
    components = prof.get("components") or []
    if not components:
        print(tr(language_of(args), 'cli.generate.msg27'))
        return 0
    ...
    if source and not re.fullmatch(r"[A-Za-z0-9][\w-]{0,79}", source):
        print(tr(language_of(args), 'cli.generate.msg29', arg=repr(source)), file=sys.stderr)
        return 1
    ...
    mode = (tr(language_of(args), "cli.generate.mode_written") if args.write
           else tr(language_of(args), "cli.generate.mode_dry_run"))
    print(tr(language_of(args), 'cli.generate.msg31', mode=mode, count=len(written), items=', '.join(written) or tr(language_of(args), 'cli.common.none')))
    if skipped:
        print(tr(language_of(args), 'cli.generate.msg32', count=len(skipped), items=', '.join(skipped)))
    if bad:
        print(tr(language_of(args), 'cli.generate.msg33', count=len(bad), items=', '.join(bad)))
    if source:
        label = tr(language_of(args), 'cli.generate.msg34') if args.write else tr(language_of(args), 'cli.generate.msg35')
        print(tr(language_of(args), 'cli.generate.msg36', label=label, source=source, count=len(seeded), items=', '.join(seeded) or tr(language_of(args), 'cli.common.none')))
        for note in seed_notes:
            print(note)
        if not seeded:
            # 0 건을 조용히 성공으로 끝내면 승계가 끝났다고 오인한다. 이유를 못박아 출력한다.
            print(tr(language_of(args), 'cli.generate.msg37'))
        print(tr(language_of(args), 'cli.generate.msg38'))
        print(tr(language_of(args), 'cli.generate.msg39'))
        print(tr(language_of(args), 'cli.generate.msg40'))
        return 0
    print(tr(language_of(args), 'cli.generate.msg41'))
    print(tr(language_of(args), 'cli.generate.msg42'))
    print(tr(language_of(args), 'cli.generate.msg43'))
    print(tr(language_of(args), 'cli.generate.msg44'))
    print(tr(language_of(args), 'cli.generate.msg45'))
    print(tr(language_of(args), 'cli.generate.msg46'))
    return 0
...
def _gen_mcp(args, root):
    ...
    if args.id:
        if args.id not in all_spec_ids:
            print(tr(language_of(args), 'cli.generate.msg47', args_id=args.id), file=sys.stderr)
            return 2
        target_ids = [args.id]
    else:
        prof = _load_profile_dict(root, args.dest)
        ...
        if enabled is not None:
            ...
            if missing:
                print(tr(language_of(args), 'cli.generate.msg48', items=', '.join(missing)), file=sys.stderr)
                return 1
    ...
    if not target_ids:
        print(tr(language_of(args), 'cli.generate.msg49'))
        return 0
    ...
    for sid in target_ids:
        ...
        models.append(mdl)
    if had_fail:
        print(tr(language_of(args), 'cli.generate.msg51'), file=sys.stderr)
        return 1
    ...
    if "codex" in targets and any("codex" in m["runtime_targets"] for m in models):
        ...
        if collide:
            print(tr(language_of(args), 'cli.generate.msg52', items=', '.join(collide)), file=sys.stderr)
            return 1
    ...
    if args.write and not write_host:
        print(tr(language_of(args), 'cli.generate.msg55'))
    if write_host:
        ...
        try:
            ...
            for tmp, outp, label in staged:
                os.replace(tmp, outp)
                owner = (tr(language_of(args), "cli.generate.mcp_owner_write_guard") if label == "claude"
                        else tr(language_of(args), "cli.generate.mcp_owner_managed_block"))
            ...
            for tmp, _o, _l in staged:
                try:
                    ...
                    pass
            print(tr(language_of(args), 'cli.generate.msg56', e=e), file=sys.stderr)
            return 1
    else:
    ...
    if args.write:
        ...
        try:
            ...
            print(tr(language_of(args), 'cli.generate.msg57', exc=exc), file=sys.stderr)
            return 1
        print(tr(language_of(args), 'cli.generate.msg58'))
    return 0
...
def _extract_config_from_profile(prof, root, dest, language=None):
    ...
    if ref:
        try:
            ...
            print(tr(language, 'cli.generate.msg59', ref=ref, arg=type(e).__name__, e=e),
                  file=sys.stderr)
    return cfg or None
...
def _gen_interpretive(args, root, kind):
    ...
    scripts_dir = os.path.dirname(_resources.hooks_src_dir())   # 엔진 소스 scripts/sage_harness (드라이버 위치)
    ...
    print(tr(language_of(args), 'cli.generate.msg60', kind=kind, count=len(ids)))
    if not ids:
        print(tr(language_of(args), 'cli.generate.msg61', kind=kind))
        return 0

    # profile 1회 로드 — 추출 config 파생(P1) + deploy host/prefix 양쪽에서 재사용.
    prof = _load_profile_dict(root, args.dest) or {}
    config = _extract_config_from_profile(prof, root, dest, language_of(args))
    if config is None:
        print(tr(language_of(args), 'cli.generate.msg62'))
    ...
    if deploy_codex and kind != "skill":
        print(tr(language_of(args), 'cli.generate.msg63'), file=sys.stderr)
        deploy_codex = False
    prefix = ""
    if deploy_codex:
        from sage.runtime_hosts import active_host
        ...
        if host != "codex":
            ...
            print(tr(language_of(args), 'cli.generate.msg64'),
                  file=sys.stderr)
            deploy_codex = False
        else:
            ...
            if not prefix:
                ...
                print(tr(language_of(args), 'cli.generate.msg65'),
                      file=sys.stderr)
                return 1
            if not re.match(r"^[A-Za-z0-9_-]+$", prefix):
                # 경로 탈출 방어(codex 리뷰 P2): prefix 가 전역 경로 조립에 들어가므로 / · .. 등 차단.
                print(tr(language_of(args), 'cli.generate.msg66', prefix=prefix),
                      file=sys.stderr)
                return 1
    ...
    for aid in ids:
        ...
        if missing:
            print(tr(language_of(args), 'cli.generate.msg69', aid=aid, items=', '.join(missing)), file=sys.stderr)
            print(tr(language_of(args), 'cli.generate.msg70', aid=aid), file=sys.stderr)
            failed.append(aid); continue
        try:
            spec_md, claims_yaml, claims = drv.extract(aid, claude_r, codex_r, guide, config)
        except Exception as e:
            print(tr(language_of(args), 'cli.generate.msg71', aid=aid, arg=type(e).__name__, e=e), file=sys.stderr)
            failed.append(aid); continue
        if args.write:
            ...
            print(tr(language_of(args), 'cli.generate.msg72', aid=aid, count=len(claims['required_claims']), count2=len(claims['unresolved'])))
            ...
            if deploy_codex:
                ...
                if status == "installed":
                    print(tr(language_of(args), 'cli.generate.msg73', gdst=gdst))
                    deployed.append(aid)
                elif status == "missing":
                    print(tr(language_of(args), 'cli.generate.msg74', os_path=os.path.relpath(codex_r, dest)), file=sys.stderr)
                else:
                    print(tr(language_of(args), 'cli.generate.msg75', status=status, gdst=gdst), file=sys.stderr)
        else:
            print(tr(language_of(args), 'cli.generate.msg76', aid=aid, count=len(claims['required_claims']), count2=len(claims['unresolved']))
                  + (tr(language_of(args), 'cli.generate.msg77') if deploy_codex else ""))
        written.append(aid)

    if deployed:
        print(tr(language_of(args), 'cli.generate.msg78', count=len(deployed), prefix=prefix))
    if failed:
        print(tr(language_of(args), 'cli.generate.msg79', count=len(failed), items=', '.join(failed)), file=sys.stderr)
        return 1
    return 0
...
def run(args) -> int:
    ...
    if not root:
        print(tr(language_of(args), 'cli.generate.msg80'), file=sys.stderr)
        return 2
    ...
    if rc == 0 and args.write:
        try:
            ...
            print(tr(language_of(args), 'cli.generate.msg81', exc=exc), file=sys.stderr)
            return 1
    return rc
