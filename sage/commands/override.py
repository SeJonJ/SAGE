"""sage override — 게이트 BLOCK 합법 우회 grant + append-only 감사 (외부검토 P1-5).

게이트(pre-implementation-gate · pre-phase4-checklist-gate)가 BLOCK 하는데 운영상 정당한 우회가
필요할 때, 사유(--reason)와 기한(--ttl)을 명시해 시한부 권한을 grant 한다. 권한과 그 권한이 실제로
통과시킨 BLOCK(bypass)은 모두 <root>/.sage/override.jsonl 에 append-only 로 남아 사후 추적된다.
TTL 만료로 자동 회수되므로 상시 우회가 불가능하다.
...
def run(args):
    ...
    if args.list:
        ...
        for g in active:
            print(f"   - gate={g['gate']} | 만료 {g['expires_at']} | 사유: {g.get('reason')} | by {g.get('user')}")
