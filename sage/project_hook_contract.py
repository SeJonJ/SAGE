...
def inspect_project_hook(root, hook_id):
    ...
    # **레이아웃을 본다.** 여기서 신 경로를 고정하면 이행 전 프로젝트의 project hook 이
    # "core 없음" 으로 판정된다 — 파일은 구 자리에 멀쩡히 있는데. 사용자가 쓴 게이트가
    # 도구 때문에 사라진 것처럼 보이는 상태다.
    core_path = os.path.join(asset_paths.layout_hooks_dir(root, asset_paths.detect_layout(root)),
...
def adapter_body(host, hook_id, hooks_rel):
    """project hook adapter 본문. `hooks_rel` 은 호출부가 판정한 레이아웃의 hook 트리 상대경로다.

    경로를 여기서 정하지 않는다 — 저장 위치는 레이아웃을 따르는데 본문이 고정 경로를 적으면, 구
    레이아웃 설치본에서 없는 트리를 가리키는 adapter 가 생긴다. 구분자는 bash 가 읽을 본문이라
    `/` 로 적는다. v1.0.0 이 그렇게 적었고, 같은 바이트여야 기존 adapter 가 그대로 정본이다.
    """
    root_env = "CLAUDE_PROJECT_DIR" if host == "claude" else "CODEX_PROJECT_ROOT"
    hooks_rel = hooks_rel.replace("\\", "/")
    return (
        "#!/bin/bash\n"
        ...
        'CORE_DIR="$PROJECT_ROOT/' + hooks_rel + '"\n'
