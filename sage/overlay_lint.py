...
# (id, 정규식). **사람 설명은 여기 없다** — 문안은 catalog 가 `lint.<id>` 로 소유한다.
# 정규식 안의 한국어는 화면 문장이 아니라 **탐지 대상**이다. 번역하면 한국어로 쓰인 게이트
# 완화 표현을 더 이상 잡지 못한다 — 조용히 통과하는 오버레이가 생긴다.
...
_COMPILED = [(pid, re.compile(pat, re.IGNORECASE)) for pid, pat in _GATE_RELAX_PATTERNS]
...
def scan_text(text):
    """오버레이 본문에서 게이트-완화 의심 표현을 찾아 [(pattern_id, 진단)] 반환(중복 제거).

    pattern_id 는 그대로 두고 설명만 진단으로 바꾼다 — id 는 strict 승격·감사에서 쓰이는
    기계 값이고, 설명은 사람이 읽는 문장이라 소유자가 다르다.
    """
    hits = []
    seen = set()
    for pid, rx in _COMPILED:
        if rx.search(text) and pid not in seen:
            seen.add(pid)
            hits.append((pid, Diagnostic(f"lint.{pid}")))
    return hits
...
def scan_overlays(root):
    ...
    반환 [(relpath, [(pattern_id, 진단), ...]), ...] — 매칭된 파일만. 디렉토리 없으면 빈 리스트.
    """
...
def _split_frontmatter(text):
    if not text.startswith("---\n"):
        return None, text, Diagnostic("lint.frontmatter_missing")
    end = text.find("\n---\n", 4)
    if end < 0:
        return None, text, Diagnostic("lint.frontmatter_unterminated")
    try:
        meta = yaml.safe_load(text[4:end]) or {}
    except yaml.YAMLError as e:
        return None, text, Diagnostic("lint.frontmatter_unparsable", evidence=str(e))
    if not isinstance(meta, dict):
        return None, text, Diagnostic("lint.frontmatter_not_mapping")
    ...
    for fn in sorted(os.listdir(base)):
        ...
        try:
            ...
            findings.append(("critical-domain-drift", rel,
                             Diagnostic("lint.override_unreadable", evidence=str(e))))
            continue
        ...
        if unknown_keys:
            findings.append(("critical-domain-drift", rel,
                             Diagnostic("lint.frontmatter_unknown_keys", keys=unknown_keys)))
        refs = meta.get("domain_refs")
        if not isinstance(refs, list) or not all(isinstance(x, str) and x for x in refs):
            findings.append(("critical-domain-drift", rel,
                             Diagnostic("lint.domain_refs_not_string_list")))
            continue
        unknown = sorted(set(refs) - set(registry))
        if unknown:
            findings.append(("critical-domain-drift", rel,
                             Diagnostic("lint.domain_refs_unregistered", refs=unknown)))
            ...
            for field in ("path_globs", "content_keywords"):
                for trigger in domain.get(field) or []:
                    ...
                    if token and token in norm_body:
                        findings.append(("critical-domain-drift", rel,
                                         Diagnostic("lint.trigger_duplicated", ref=ref,
                                                    field=field, trigger=repr(trigger))))
    return findings
