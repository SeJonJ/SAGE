...
8. **Fallback**: retry the peer path or a fresh session (or, as a recorded per-round decision, `sage cross-check --on-peer-failure same-runtime`, which is recorded as degraded); if context is too large, retry with 04 Review Context + 01/02/03 + core files; if a mandatory L3 review cannot complete, record BLOCKED in 05 and do not write 06.
