...
**CORE framework bootstrap assets** — the `sage-pdca-start` / `sage-review` / `sage-asset` /
...
`$CODEX_HOME/skills/` (`$sage-init`, `$sage-pdca-start`, `$sage-review`, `$sage-asset`, `$sage-profile-modify`); and since Codex
...
- Do not directly edit generated artifacts (`{host}/agents`, `{host}/skills`,
  ...
  `sage-review`/`sage-asset`/`sage-profile-modify` skills and the six CORE roster agent
