...
self-contained: 합성 입력 + 예시 인스턴스 config (특정 소비 프로젝트 비의존 = 독립).
"""
import os
import sys
import unittest
...
from extract_config_example import EXAMPLE_EXTRACT_CONFIG as CFG  # noqa: E402  (예시 인스턴스 config 주입)

GUIDE = "Do not run git commit or git push. Do not edit generated/ directly."
...
1. 구현 가이드 작성
frontend 수정 금지
ONLY_IN_CLAUDE: backend/src/main/java/onlyclaude
"""
...
class TestExtract(unittest.TestCase):
    ...
    def test_intersection_high(self):
        ...
        wc = [c for c in self.req if c["value"] == "backend/src/main/java/core"]
    ...
    def test_one_sided_unresolved(self):
        ...
        self.assertIn("backend/src/main/java/onlyclaude", self.claims["unresolved"])
...
class TestSpecDraft(unittest.TestCase):
    def test_draft_skeleton(self):
        ...
        draft = rx.spec_draft("backend-expert", CLAUDE, CODEX, claims)
...
class TestDeterminism(unittest.TestCase):
    ...
    def test_engine_domain_free(self):
        # 제약 #2(독립): config 없이는 owned_paths 미추출 (엔진에 프로젝트 경로 하드코딩 없음)
