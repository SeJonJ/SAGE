...
# adapter: session JSONL 읽기 → snapshot / 경로·env / report 파일쓰기 / report path 출력. 집계는 core.
# policy_delta(output_contract/knowledge_capture) 미병합 — Codex-only/OPTION 정책모듈 보존만. (직접수정 금지)
...
# NOTE: output_contract_check / knowledge_capture 정책은 v1 미병합(policies/ 보존). policy_results 비움.
