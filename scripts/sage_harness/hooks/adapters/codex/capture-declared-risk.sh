#!/bin/bash
# capture-declared-risk — Codex thin adapter (R1: 본문은 runtime/run_hook.py 단일소스)
...
# log_dir 는 run_hook 이 root/.codex/logs 로 해석. (generated artifact — 직접수정 금지)
...
exec python3 "$CORE_DIR/runtime/run_hook.py" \
  --runtime codex --hook capture-declared-risk --root "$PROJECT_ROOT" --core-dir "$CORE_DIR"
