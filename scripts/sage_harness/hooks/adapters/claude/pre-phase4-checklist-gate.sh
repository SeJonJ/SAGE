#!/bin/bash
# pre-phase4-checklist-gate — Claude thin adapter (R1: 본문은 runtime/run_hook.py 단일소스)
...
exec python3 "$CORE_DIR/runtime/run_hook.py" \
  --runtime claude --hook pre-phase4-checklist-gate --root "$PROJECT_ROOT" --core-dir "$CORE_DIR"
