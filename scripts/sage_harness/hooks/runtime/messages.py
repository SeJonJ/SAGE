...
def _gate_record(decision, profile):
    ...
    return {
        ...
        "block_phase00_mixed_evidence": ("BLOCK", "PDCA", f"{rs}.", False,
                             "Phase 00 수정부터 별도 변경으로 완료한 뒤 영향 Phase를 순서대로 재실행하세요"),
        "warn_phase00_mixed_evidence": ("WARN", "PDCA", f"{rs}.", False,
        ...
        "block_invalid_done_criteria": ("BLOCK", "PDCA", f"{rs}.", False,
                             "Phase 00 Done Criteria 구조와 Done-Criteria-Revision을 먼저 고치세요"),
        "warn_invalid_done_criteria": ("WARN", "PDCA", f"{rs}.", False,
                             "(advisory) Phase 00 Done Criteria 구조를 보완하세요"),
        "warn_done_criteria_progress": ("WARN", "PDCA", f"{rs}.", False,
                             "현재 Phase에서 실제 충족된 항목만 [x], 범위 밖은 사유 있는 [~]로 갱신하세요"),
        "block_stale_done_criteria_revision": ("BLOCK", "PDCA", f"{rs}.", False,
                             "Phase 00에 선언한 영향 Phase를 재실행하고 현재 revision을 기록하세요"),
        "warn_stale_done_criteria_revision": ("WARN", "PDCA", f"{rs}.", False,
                             "(advisory) 영향 Phase를 재실행하고 현재 revision을 기록하세요"),
        "block_report_without_done_criteria": ("BLOCK", "PDCA", f"{rs}.", False,
                             "Phase 00을 먼저 수정해 충족 항목은 [x], 실제 범위 밖은 사유 있는 [~]로 해결하세요"),
        "warn_report_without_done_criteria": ("WARN", "PDCA", f"{rs}.", False,
                             "(advisory) Phase 06 전에 Phase 00 Done Criteria를 모두 해결하세요"),
        "block_stale_done_criteria_approval": ("BLOCK", "PDCA", f"{rs}.", False,
                             "Phase 00 변경 뒤 새 review loop를 실행하고 Phase 05 승인을 다시 작성하세요"),
        "warn_stale_done_criteria_approval": ("WARN", "PDCA", f"{rs}.", False,
