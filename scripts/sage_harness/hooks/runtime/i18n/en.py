"""hook English catalog.

Key set and named placeholders must match `ko.py` exactly. `{rs}` carries the language-neutral
reason string the decision core produced and is inserted verbatim, never translated here.
"""

MESSAGES = {
    "block_desktop": "Direct edits to synchronized artifacts and forbidden paths are not allowed.",
    "block_feedback_unresolved": "New implementation on top of an unresolved developer feedback marker (!sage-feedback) is not allowed.",
    "block_l3_no_plan": "L3 work with no plan document.",
    "block_l3_strategy_unresolved": "The L3 review matching strategy is unresolved, so the review cannot be confirmed.",
    "block_l3_review_evidence": "L3 review evidence for the current cycle is incomplete.",
    "warn_l3_no_review": "The two-round review document was not found.",
    "warn_l2_no_plan": "A source or configuration change with no plan document.",
    "warn_l0_l3_content": "L3 content keywords detected in a document or plan — check for exposed sensitive information.",
    "block_phase_incomplete": "Mandatory PDCA phases not written: [{miss}].",
    "warn_phase_incomplete": "Recommended PDCA phases not written: [{miss}].",
    "block_cycle_risk_declaration": "Cycle `{cycle_stem}` has no valid Phase 00 Risk Level declaration.",
    "block_cycle_risk_reconciliation": "The computed risk {required_risk} is higher than the Phase 00 Risk Level {phase00_risk}.",
    "block_report_without_approval": "{rs}.",
    "block_report_mixed_evidence": "{rs}.",
    "block_phase00_mixed_evidence": "{rs}.",
    "warn_phase00_mixed_evidence": "{rs}.",
    "block_report_without_audit": "{rs}.",
    "warn_report_without_audit": "{rs}.",
    "block_invalid_done_criteria": "{rs}.",
    "warn_invalid_done_criteria": "{rs}.",
    "warn_done_criteria_progress": "{rs}.",
    "block_stale_done_criteria_revision": "{rs}.",
    "warn_stale_done_criteria_revision": "{rs}.",
    "block_report_without_done_criteria": "{rs}.",
    "warn_report_without_done_criteria": "{rs}.",
    "block_stale_done_criteria_approval": "{rs}.",
    "warn_stale_done_criteria_approval": "{rs}.",
    "block_cycle_binding": "{rs}.",
    "block_document_language_conflict": "cycle `{cycle_stem}` declares conflicting document languages.",
    "warn_document_language_missing": "cycle `{cycle_stem}` has documents without a language declaration.",
    "block_document_prose_language": "cycle `{cycle_stem}`'s new/changed document prose still contains text that is not in the declared language.",
    "block_document_prose_structure": "cycle `{cycle_stem}`'s document has no prose written in the declared language (ko).",
    "block_document_unclosed_fence": "cycle `{cycle_stem}`'s document has an unclosed code fence.",
    "block_document_post_image": "The document that this edit would produce could not be reconstructed, so its prose language cannot be judged.",
    "block_prose_scanner_unavailable": "The prose language scanner could not be loaded — the installation may be damaged.",
    "block_cycle_closed": "Cycle `{cycle_stem}` is already closed — a new source edit cannot be bound to it.",
    "block_report_without_acceptance": "{rs}.",
    "warn_report_without_acceptance": "{rs}.",
    "warn_report_with_l3_waiver": "{rs}.",
    "block_report_waiver_audit_failure": "{rs}.",
    "block_fast_cycle_audit": "Fast Cycle audit or composite binding failed: {rs}.",
    "warn_fast_cycle": "Running the shortened Fast Cycle procedure: {rs}.",
    "block_cycle_stem_audit_failure": "{rs}.",
    "block_gate_runtime_error": "{rs}.",
    "ok_l3": "review confirmed",
    "ok_l2": "plan confirmed",
    "ok_l1": "binding confirmed",
    "ok_l0": "binding confirmed",

    "cycle_state.read_failed": "{path}: failed to read ({error_type})",
    "cycle_state.json_invalid": "{path}: JSON parse failed — corrupted or hand-edited",
    "cycle_state.not_object": "{path}: top level is not an object",
    "cycle_state.stem_invalid": "{path}: cycle_stem is missing or invalid",
    "cycle_state.reread_failed": "{path}: re-read failed ({error_type})",
    "cycle_state.language_invalid": "{path}: document_language is invalid ({language!r})",
    "cycle_declaration_ignored": "[cycle declaration ignored] {detail} — proceeding as if "
        "nothing was declared. Rewrite it with `sage cycle set <stem>` or clear it with "
        "`sage cycle clear`.",
}

# 판정 문장이 아니라 그 문장을 **감싸는 조각**이다. message_key 와 섞으면 build-time oracle
# (core 가 낼 수 있는 key 집합과의 대조)이 조각까지 message_key 로 세어 무너진다.
FRAGMENTS = {
    # 판정 줄에 이어 붙는 해소 안내. 본문만 번역하고 이걸 두면 en 출력의 절반이
    # 한국어로 남아, 번역하지 않은 것보다 읽기 어려운 줄이 된다.
    # key 는 `hint.<message_key>` 규약이다 — 표를 한 벌 더 두지 않기 위해서다.
    "hint.block_feedback_unresolved": "Resolve the marker with `/sage-feedback` (an edit that removes it passes), or issue a waiver recording the reason and a deadline if this is urgent",
    "hint.block_l3_no_plan": "Author a plan document and run the L3 review protocol (two rounds)",
    "hint.block_l3_strategy_unresolved": "(override required: select a canonical find_l3_review strategy in the SAGE manifest)",
    "hint.block_l3_review_evidence": "Check the review frontmatter, domain_ref and round [1, 2] for this Cycle-Stem",
    "hint.block_report_without_approval": "Record APPROVED in the approve phase document before writing the report",
    "hint.block_report_mixed_evidence": "Finish the 01/04/05 changes first, then write the 06 report as a separate change",
    "hint.block_phase00_mixed_evidence": "Complete the Phase 00 fix as its own change, then re-run the affected phases in order",
    "hint.warn_phase00_mixed_evidence": "(advisory) Split Phase 00 and the phases that follow it into separate changes",
    "hint.block_report_without_audit": "Run Phase 05 through {rv} to close the loop (APPROVED) and record 'Loop-Run: <run_id>' in the 05 document",
    "hint.warn_report_without_audit": "(advisory) Review-loop audit evidence is recommended for Phase 05 — run the loop with {rv} and record 'Loop-Run: <run_id>' in 05",
    "hint.block_invalid_done_criteria": "Fix the Phase 00 Done Criteria structure and Done-Criteria-Revision first",
    "hint.warn_invalid_done_criteria": "(advisory) Improve the Phase 00 Done Criteria structure",
    "hint.warn_done_criteria_progress": "Mark [x] only for items actually met in this phase, and [~] with a reason for what is out of scope",
    "hint.block_stale_done_criteria_revision": "Re-run the affected phases declared in Phase 00 and record the current revision",
    "hint.warn_stale_done_criteria_revision": "(advisory) Re-run the affected phases and record the current revision",
    "hint.block_report_without_done_criteria": "Fix Phase 00 first: [x] for what is met, [~] with a reason for what is genuinely out of scope",
    "hint.warn_report_without_done_criteria": "(advisory) Resolve every Phase 00 Done Criteria item before Phase 06",
    "hint.block_stale_done_criteria_approval": "Run a new review loop after the Phase 00 change and write the Phase 05 approval again",
    "hint.warn_stale_done_criteria_approval": "(advisory) The Phase 00 change made the existing approval stale. Review Phase 05 again",
    "hint.block_document_language_conflict": "Take the `Document-Language:` line in Phase 00 as the source of truth, align the cycle's other documents to it, and update the .sage/cycle.json mirror with `sage cycle set <stem> --document-language <ko|en>`",
    "hint.block_document_prose_language": "Rewrite the prose in the declared language — quoting external evidence verbatim inside a blockquote (`>`) or a code fence keeps it out of the check",
    "hint.block_document_prose_structure": "Write the prose in the declared `Document-Language: ko`, or change the Phase 00 declaration to `en` if this document really is meant to be English",
    "hint.block_document_unclosed_fence": "Close the code fence opened on that line with the same character and an equal or greater length — while it stays open the rest of the document is treated as code and silently skips the prose language check",
    "hint.block_document_post_image": "The partial edit's context does not match what is on disk — re-read the file and edit again, or rewrite the whole document (Write / apply_patch Add File)",
    "hint.block_prose_scanner_unavailable": "Check the hook runtime integrity with `sage validate` and repair the installation with `sage install --force`",
    "hint.warn_document_language_missing": "(advisory) Add one `Document-Language: <ko|en>` line to each document of this cycle",
    "hint.block_report_without_acceptance": "Record acceptance evidence (PASS/FAIL/NOT TESTED/N/A) in 04-analyze and review 05 again",
    "hint.warn_report_without_acceptance": "(advisory) Strengthen the acceptance evidence in 04-analyze",
    "hint.warn_report_with_l3_waiver": "Record the remaining evidence after operational verification and revoke the waiver",
    "hint.block_report_waiver_audit_failure": "Check write permission and integrity for .sage/acceptance-waivers.jsonl",
    "hint.block_fast_cycle_audit": "Restore the Fast Plan and .sage/fast_cycle.jsonl, then confirm with `sage fast-cycle show`",
    "hint.warn_fast_cycle": "This is audited to .sage/fast_cycle.jsonl and carries a weaker verification guarantee than the standard procedure",
    "hint.block_cycle_stem_audit_failure": "Check write permission and integrity for .sage/override.jsonl — a declared cycle stem that cannot be recorded cannot be allowed through unaudited",
    "hint.block_gate_runtime_error": "Check the profile types and the integrity of the installed SAGE runtime, then run validate again",
    "gate.frame.file": "file",
    "gate.frame.reason": "evidence",

    # 계산 hint 의 조각. 출처에 따라 문장이 갈리므로 완성문이 아니라 조각으로 둔다.
    "hint.phase_doc": "Write the phase document first (docs/agent/pdca-templates.md)",
    "hint.binding": "Make the phase document's filename match its Cycle-Stem declaration so the current cycle resolves to exactly one",
    "hint.declare_stem": "The cycle stem was inferred from the branch leaf `{stem}` — if that is not the current cycle, declare it with `sage cycle set <stem>` (required on long-lived branches; for a single CI process use `export SAGE_CYCLE_STEM=<stem>`, audited to .sage/override.jsonl)",
    "hint.if_this_cycle": "{lead}. If this is the right cycle: {tail}",
    "hint.clear_cycle": "An already declared cycle may be the cause — check with `sage cycle show` and release it with `sage cycle clear` (or `unset SAGE_CYCLE_STEM` if it was declared through the environment)",
    "hint.binding_non_phase": "{binding}. For a non-phase edit, select the cycle with `sage cycle set <stem>` (in CI, `export SAGE_CYCLE_STEM=<stem>`). {clear}",
    "hint.binding_phase": "{binding}. {clear}",
    "hint.risk_declaration": "Record exactly one of `Risk Level: L1`, `Risk Level: L2` or `Risk Level: L3` as a single line in the Phase 00 document for this Cycle-Stem",
    "hint.raise_phase00": "If this really is {required_risk} work, raise the Phase 00 Risk Level in `{phase00_path}` to {required_risk} or higher and try again",
    "hint.risk_from_declaration": "This risk level came from a {required_risk} declaration made in this session. If it was captured by mistake, clear it by typing `위험도 선언 해제` — {raise_00}",
    "hint.cycle_closed": "Write the new cycle's Phase 00 first, then declare it with `sage cycle set <new stem>` (in CI, `export SAGE_CYCLE_STEM=<new stem>`). {clear}",
    "hint.desktop_default": "Edit the source path and re-sync",

    # 결속을 어디서 읽었는지 말하는 라벨. 통과 줄 꼬리에 붙어 사용자가 읽는다.
    "origin.env": "SAGE_CYCLE_STEM declaration",
    "origin.cli": ".sage/cycle.json declaration",
    "origin.declared": "declaration",
    "origin.branch_leaf": "branch-leaf inference",
    "origin.phase_doc": "phase document",

    # 판정 code 의 hook 렌더. CLI 와 **같은 key 를 중복 등록하지 않는다** —
    # 접두가 도메인을 가르고, 두 도메인이 같은 문장의 소유자가 되지 않는다.
    "hook.context.not_mapping": "{section} must be a mapping (object).",
    "hook.context.unknown_keys": "{section} contains unknown keys: {keys}",
    "hook.context.enabled_not_bool": "context_management.compaction.enabled must be a bool (true/false).",
    "hook.context.preserve_not_list": "context_management.compaction.preserve must be a list.",
    "hook.context.preserve_empty": "preserve cannot be empty when compaction.enabled=true.",
    "hook.context.preserve_unsupported": "context_management.compaction.preserve contains unsupported items — allowed: {allowed}",
    "hook.context.preserve_duplicated": "context_management.compaction.preserve does not allow duplicates.",
    "hook.context.snapshot_bytes_range": "context_management.compaction.max_snapshot_bytes must be an integer in {minimum}..{maximum}.",
    "hook.feedback.git_timeout": "git ls-files did not finish in time ({root})",
    "hook.feedback.git_failed": "git ls-files could not be run ({root})",

    "hook.checklist_contract.glob_empty": "glob must be a non-empty string",
    "hook.checklist_contract.glob_too_long": "glob must be at most 512 characters",
    "hook.checklist_contract.glob_control_chars": "glob cannot contain control characters or line breaks",
    "hook.checklist_contract.glob_absolute_path": "glob cannot be a POSIX/Windows absolute, rooted, or drive-relative path",
    "hook.checklist_contract.glob_dotdot": "glob cannot contain a '..' path segment",
    "hook.checklist_contract.targets_not_list": "checklist_scan_targets must be a list",
    "hook.checklist_contract.target_not_mapping": "{where} must be a label/glob mapping",
    "hook.checklist_contract.target_keys_not_string": "{where} keys must be strings",
    "hook.checklist_contract.target_unknown_keys": "{where} has unknown keys {unknown}; only label/glob/is_impl are allowed",
    "hook.checklist_contract.target_label_invalid": "{where}.label must be a single-line string of 1..80 characters",
    "hook.checklist_contract.target_glob_invalid": "{where}.glob: {reason}",
    "hook.checklist_contract.target_is_impl_not_bool": "{where}.is_impl must be a bool",

    "hook.override_audit.state_home_not_absolute": "Cannot determine the permission-cache location (not an absolute path: {candidate}). This looks like an environment with HOME unset — set {env_var} to an absolute path",
    "hook.override_audit.git_neither_file_nor_dir": "`.git` is neither a file nor a directory: {git}",
    "hook.override_audit.git_pointer_unreadable": "cannot read the `.git` pointer ({git}): {error_type}: {exc}",
    "hook.override_audit.git_pointer_bad_format": "`.git` pointer is not in the expected format ({git})",
    "hook.override_audit.git_pointer_target_missing": "the gitdir the `.git` pointer refers to does not exist ({target})",
    "hook.override_audit.repo_boundary_unresolved": "cannot determine the repository boundary — {reason}. Issuing a grant without a resolved identity risks mixing it with another repository's permissions",
    "hook.override_audit.identity_marker_create_failed": "cannot create the repository identity marker ({marker}): {error_type}: {exc}. Issuing without an identity means a different repository later created at the same path would inherit this permission",
    "hook.override_audit.identity_marker_unreadable": "cannot read the repository identity marker ({marker})",
    "hook.override_audit.state_home_inside_repo": "the permission-cache location is inside the repository ({home} ⊂ {root}). If committed, the bypass would activate in other clones — set {env_var} outside the repository",
    "hook.feedback.git_exit": "git ls-files exited with {code}",
    "hook.feedback.unreadable_file": "a tracked file could not be read ({path}) — this is unknown, not marker-free",

    # sage-hook 진입점의 판정 code. hook_entry 는 `sage.i18n` 을 import 하지 않고
    # core_dir 에서 이 catalog 를 찾는다 — 출력 도메인이 hook 이므로 catalog 도 hook 것이다.
    "hook.entry.blocked": "[sage-hook] {hook} blocked: {detail}",
    # --- recovery step descriptions (Action steps, no command) ---
    "recovery.explain": "First check why this path carries these requirements",
    "recovery.status": "Check the current project state",
    "recovery.validate": "Check asset, schema, and hash integrity",
    "recovery.cycle_show": "Check the current cycle declaration",
    "recovery.cycle_set": "Declare the Cycle-Stem for this work",
    "recovery.cycle_clear": "Clear the completed cycle declaration",
    "recovery.write_phases": "Write the missing phase documents under the same Cycle-Stem.",
    "recovery.fix_canonical_spec": "Edit this artifact's canonical spec — the spec owns it, not the installed copy.",
    "recovery.regenerate": "Regenerate the artifact after fixing the spec",
    "recovery.reinstall_host": "Repair the installed assets",
    "recovery.fix_risk_declaration": "Correct the Risk Level declaration in the Phase 00 header so it matches the real risk.",
    "recovery.resolve_feedback": "Resolve the unresolved `!sage-feedback` markers in this file.",
    "recovery.write_plan": "Write the plan document for this work.",
    "recovery.select_l3_strategy": "Select an L3 review strategy in the profile (manifest `l3_review_strategy`).",
    "recovery.run_review": "Run the Phase 05 review so the evidence exists",
    "recovery.fast_open": "Open a Fast Cycle",
    "recovery.fix_document": "Fix the document location that was reported.",
    "recovery.fix_shared_profile": "Fix the offending entry in the shared profile",
    "recovery.fix_report": "Fill in the missing entries of the Phase 06 report.",
    "recovery.move_off_desktop": "Move this file out of the forbidden path before working on it.",
    # --- write guard, five branches (previously assembled as Korean in code) ---
    "guard.cycle_declaration": "`{path}` holds the cycle declaration — editing it directly lets a completed cycle be named and the gate passed.",
    "guard.framework_doc": "`{path}` is a CORE framework document — `sage install --force` overwrites it. Put project values in sage/project-profile.yaml and rules in the conventions/critical-domain/project-local documents.",
    "guard.core_render": "`{path}` is a CORE bootstrap render — write project-local customization in `{overlay}` so it survives reinstallation.",
    "guard.core_render_blocked": "`{path}` is a gate-bearing CORE asset with no independent executable oracle, so overlays are unsupported for now — update the CORE base by reinstalling for the chosen host/scope.",
    "guard.generated_asset": "`{path}` is a generated artifact — editing it directly means the next generate overwrites it.",
    "hook.runtime.api_too_old": "This project's hooks require SAGE runtime API {required}, but this sage-hook provides API {current}.",
    "hook.runtime.api_marker_missing": "This is a 1.0 manifest with no runtime_api marker — a missing marker is not treated as a pre-1.0 installation.",
    "hook.runtime.api_marker_damaged": "The runtime_api marker cannot be interpreted ({reason}).",
    "hook.runtime.api_marker_absent_legacy": "This installation has no runtime_api marker — treated as a pre-1.0 install and allowed to continue ({reason}).",
    "hook.runtime.manifest_unreadable": "The install manifest is unreadable, so runtime compatibility could not be judged ({reason}).",
    "hook.runtime.required_sage": "Required SAGE: {version}",
    "hook.entry.profile_missing_damaged": "There is an installation record (manifest) but no profile — the installation is damaged. Repair it with `sage install --force`, then confirm with `sage doctor`.",
    "hook.entry.not_an_install_target": "There is no installation record (manifest) either — this directory may not be a SAGE install target. If the install was intended, run `sage install`; otherwise remove the sage-hook registration from {settings} (a hook left behind where nothing is installed blocks every edit like this).",
    "hook.entry.profile_yaml_unreadable": "The profile YAML could not be read ({path})",
    "hook.entry.compiled_profile_unreadable": "The compiled profile could not be read ({path})",
    "hook.entry.profile_not_mapping": "The profile root must be an object (mapping).",
    "hook.entry.raw_risk_type": "A raw risk field in the profile has the wrong type",
    "hook.entry.profile_pair_mismatch": "project-profile.yaml and project-profile.json disagree. Run `sage generate` again.",
    "hook.entry.stop_retry_pass": "[sage-hook] {hook} retry (stop_hook_active) — passing instead of blocking on a profile problem: {detail}",
    "hook.entry.core_load_failed": "⛔ [sage-hook] the hook core could not be loaded ({path})",
    "hook.entry.dispatch_failed": "⛔ [sage-hook] hook dispatch failed ({hook})",
    "hook.entry.remediation": "; remediation: {actions}",
    "hook.entry.usage_description": "Run a SAGE hook (cross-platform, no bash dependency)",
    "hook.entry.usage_root": "Project root (default: resolved from env/git/cwd)",
    "hook.entry.usage_core_dir": "Hook core path (default: project-local, then bundled)",
    "hook.entry.usage_path": "Compatibility path for calling the write guard directly (takes precedence over stdin JSON)",
    "hook.version.sage_section_not_mapping": "The sage section must be a mapping (object).",
    "hook.version.required_not_semver": "sage.required_version={value} is not an exact SemVer.",
    "hook.version.required_semver_example": "Example: sage.required_version: 1.2.3",
    "hook.version.required_absent": "The project declares no required SAGE version (legacy profile).",
    "hook.version.set_required": "Set sage.required_version in the shared profile to an exact version",
    "hook.version.axis_malformed": "The SAGE {axis} version is malformed ({value})",
    "hook.version.axis_unknown": "The SAGE {axis} version cannot be determined",
    "hook.version.axis_differs": "The SAGE {axis} version differs from the required one: {current} != {required}",
    "hook.install.file_changed_during_fingerprint": "A file changed while its fingerprint was being taken: {path}",
    "hook.install.input_changed_since_preflight": "An input changed after preflight: {path}",
    "hook.install.output_changed_since_preflight": "An output changed after preflight: {path}",
    "hook.install.inputs_changed_during_preflight": "Install inputs changed during preflight",
    "hook.install.path_unreadable": "A path could not be read: {path}",
    "hook.install.lock_root_unsafe": "The install lock root is not a safe directory: {path}",
    "hook.install.lock_root_foreign_owner": "The install lock root is owned by another user: {path}",
    "hook.install.lock_root_permissive": "The install lock root permissions are wider than 0700: {path}",
    "hook.install.destination_busy": "Another sage install is already running for this destination.",
    "hook.install.outputs_changed_before_commit": "Install outputs changed just before commit",
    "hook.runtime.not_mapping": "runtime must be a mapping (received: {received})",
    "hook.runtime.unknown_keys": "runtime contains unknown keys: {keys} (allowed: {allowed})",
    "hook.runtime.invalid_value": "runtime.{field}={value} — must be one of {allowed}",
    "hook.runtime.alias_conflicts_with_active": "The legacy runtime.host alias and runtime.active_host disagree — the authoritative active host is ambiguous.",
    "hook.runtime.installed_hosts_shape": "runtime.installed_hosts must be a non-empty array of unique [claude|codex] values.",
    "hook.runtime.active_host_not_installed": "runtime.active_host={value} is not in installed_hosts.",
    "hook.runtime.double_host_without_cross_model": "This is a double-host setup but options.cross_model is not true — an independent review by the opposite runtime is strongly recommended.",
    "hook.runtime.desired_hosts_mismatch": "The profile's desired_hosts={desired} disagrees with the install receipt={actual}.",
    "hook.runtime.active_host_surface_absent": "The discovery surface for active_host={value} is absent from the install receipt.",
    "hook.compile.profile_not_mapping": "The profile must be a mapping (object), but is {actual}.",
    "hook.compile.risk_not_mapping": "The risk section must be a mapping (object), but is {actual}.",
    "hook.compile.domains_not_list": "risk.domains must be a list, but is {actual}.",
    "hook.compile.domain_not_mapping": "{path} must be a mapping (object), but is {actual}.",
    "hook.compile.risk_level_invalid": "{path}.risk_level must be one of L1/L2/L3, but is {actual}.",
    "hook.compile.not_string_list": "{path} must be a list of non-empty strings, but is {actual}.",
    "hook.compile.bad_string_items": "{path} contains items that are not strings or are empty (index: {indexes}).",
    "hook.overlay.read_failed": "Could not read the overlay/render: {path}",
    "hook.overlay.reserved_token_in_body": "The overlay body contains the reserved marker token '{token}', so it cannot be composed.",
    "hook.routing.reserved_token_in_body": "The routing block body contains the reserved marker token '{token}', so it cannot be generated.",
    "hook.overlay.marker_duplicated": "The overlay markers are duplicated — there is more than one managed block.",
    "hook.overlay.marker_unpaired": "The overlay markers are unpaired (malformed).",
    "hook.overlay.marker_unextractable": "The overlay markers are out of order or nested, so the START..END span cannot be extracted.",
    "hook.routing.marker_duplicated": "The routing markers are duplicated — there is more than one managed block.",
    "hook.routing.marker_unpaired": "The routing markers are unpaired (malformed).",
    "hook.routing.marker_unextractable": "The routing markers are out of order or nested, so the START..END span cannot be extracted.",
    "hook.overlay.markers_interleaved": "The overlay and routing markers are interleaved, so the managed spans overlap.",
    "hook.overlay.installed_span_broken": "The installed file has a broken managed span: {path} ({reason})",
    "hook.routing_input.not_string": "must be a non-empty string.",
    "hook.routing_input.multiline_or_control": "must not contain newlines or control characters (must be a single line).",
    "hook.routing_input.path_unsafe": "only safe relative path segments are allowed (an optional leading dot followed by alphanumerics per segment; no absolute paths, backticks, spaces, markdown, or backslashes).",
    "hook.routing_input.path_parent_segment": "must not contain a parent-path (..) segment.",
    "hook.routing_input.backtick_forbidden": "must not contain a backtick (blocks code-span breakout).",
    "hook.routing_input.label_too_long": "The label must be at most {limit} characters (routing labels are short).",
    "hook.routing_input.label_markdown_char": "must not contain markdown-active characters (` * _ [ ] < > # | \\ ~) — this blocks instruction/markdown injection.",
    "hook.routing_input.label_url_scheme": "must not contain a URL scheme (://) — this blocks GFM autolink injection.",
    "hook.routing_input.path_not_found": "The path does not exist (a project-relative file): {path}",
    "hook.routing_input.not_list": "must be a list.",
    "hook.routing_input.not_mapping": "must be a mapping (object).",
    "hook.routing_input.unknown_keys": "Unknown keys: {keys} (only doc/label are allowed)",
    "hook.routing_input.risk_level_invalid": "must be one of L1/L2/L3.",
    "hook.overlay.filename_nonstandard": "Non-canonical overlay filename: '{actual}' (expected: '{expected}')",
    "hook.routing_input.field_null": "{where} must not be null (omit the key or use {empty} to leave it unset).",
    "hook.routing_input.not_mapping_at": "{where} must be a mapping (object).",
    "hook.routing_input.field_invalid": "Routing input error ({where}): {reason}",
    "hook.materialize.path_outside_root": "The project input path is outside the root: {path}",
    "hook.materialize.path_filesystem_mismatch": "The project input path is on a different filesystem than the root: {path}",
    "hook.materialize.path_stat_failed": "Failed to stat the project input path: {path}",
    "hook.materialize.path_symlink_forbidden": "A symlink is not allowed in the project input: {path}",
    "hook.materialize.path_ancestor_not_dir": "An ancestor of the project input is not a directory: {path}",
    "hook.materialize.path_not_regular_file": "The project input is not a regular file: {path}",
    "hook.materialize.path_not_dir": "The project input is not a directory: {path}",
    "hook.materialize.profile_not_mapping": "The profile's top level is not a mapping (object): {path}",
    "hook.materialize.profile_compile_failed": "Failed to materialize the profile ({path}): {evidence}",
    "hook.materialize.profile_yaml_json_mismatch": "project-profile.yaml and project-profile.json disagree — re-run sage generate.",
    "hook.materialize.profile_load_failed": "Failed to load the profile ({kind})",
    "hook.overlay.unknown_core_asset": "Unknown or misspelled CORE asset overlay: '{id}' is not a CORE {kind}.",
    "hook.overlay.unsupported_blocked": "{kind}/{id} does not support overlays (blocked — not yet backed by an independent gate oracle).",
    "hook.materialize.routing_profile_load_failed": "Failed to load the profile for the routing block: {reason}",
    "hook.overlay.unknown_core_asset_at": "Unknown or misspelled CORE asset overlay: {path} ('{id}' is not a CORE {kind})",
    "hook.overlay.unsupported_blocked_remove": "{kind}/{id} does not support overlays (blocked — not yet backed by an independent gate oracle): {path} must be removed.",
    "hook.materialize.core_render_missing": "The CORE render is missing: {path}",
    "hook.materialize.anchor_missing": "The core_renders anchor is missing or corrupt: {anchor_key}",
    "hook.materialize.base_drift": "Base drift/tampering detected: {path} (anchor mismatch)",
    "hook.overlay.expected_block_failed": "{path}: {reason}",
    "hook.overlay.block_stale": "The overlay is not applied or is stale: {path} (`sage sync-overlays` is required)",
    "hook.routing_input.compute_failed": "{path}: failed to compute the routing block ({reason})",
    "hook.routing_input.block_stale": "The routing block is not applied or is stale: {path} (`sage sync-overlays` is required)",
    "hook.materialize.stale_sage_version": "{anchor_key} was installed with SAGE {installed} (current: {current}) — upgrade with `sage install --force`.",
    "hook.lint.skip-gate": "Reads as an instruction to skip a phase, review, verification, or gate.",
    "hook.lint.bypass-gate": "Reads as an instruction to bypass a gate or guard.",
    "hook.lint.disable-gate": "Reads as an instruction to disable a gate, guard, or hook.",
    "hook.lint.ignore-guide": "Reads as an instruction to ignore AGENT_GUIDE.",
    "hook.lint.skip-phase-num": "Reads as an instruction to skip a specific phase (00-06).",
    "hook.lint.relax-ko": "This is a gate bypass or disablement expression.",
    "hook.lint.skip-review-ko": "This is an expression that omits review, verification, or approval.",
    "hook.lint.optional-gate": "Reads as making a phase, review, verification, gate, or approval optional or unnecessary.",
    "hook.lint.without-approval": "Reads as an instruction to proceed without approval, review, or verification.",
    "hook.lint.optional-gate-ko": "Reads as making a step, review, verification, gate, or approval optional or unnecessary.",
    "hook.lint.omit-gate": "Reads as an instruction to omit a phase, review, verification, gate, or approval.",
    "hook.lint.frontmatter_missing": "The YAML frontmatter (---) is missing.",
    "hook.lint.frontmatter_unterminated": "The YAML frontmatter is not terminated with ---.",
    "hook.lint.frontmatter_unparsable": "Could not parse the YAML frontmatter",
    "hook.lint.frontmatter_not_mapping": "The YAML frontmatter must be a mapping (object).",
    "hook.lint.override_unreadable": "Could not read the override",
    "hook.lint.frontmatter_unknown_keys": "The framework override frontmatter has keys that are not allowed: {keys} (only domain_refs is allowed).",
    "hook.lint.domain_refs_not_string_list": "domain_refs must be a list of non-empty strings.",
    "hook.lint.domain_refs_unregistered": "These domain_refs are not registered: {refs}",
    "hook.lint.trigger_duplicated": "The {field} trigger of domain '{ref}' must not be duplicated here: {trigger}",
    "hook.overlay.gate_relaxation": "Suspected gate relaxation [{pattern}]: {reason}",
    "hook.routing.gate_relaxation": "Suspected gate relaxation [{pattern}]: {reason}",
    "hook.context.runtime_contract_failed": "The project profile's runtime contract is not satisfied",
}
